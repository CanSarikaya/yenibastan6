...
public class WallGenerator : MonoBehaviour
{
    ...
    private void InstantiateWalls()
    {
        ...
        for (int i = 0; i < kapilist.Count; i++)
        {
            ...
            {
             Instantiate(Resources.Load("Wall_Kapili") as GameObject, new Vector3(kapilist[i]-300, 0.88f, kapilist[i + 1]-300), Quaternion.Euler(new Vector3(0, 0, 0))).transform.parent = this.transform;
            }
            Instantiate(Resources.Load("Wall_Kapili") as GameObject, new Vector3(kapilist[i], 0.88f, kapilist[i + 1]), Quaternion.Euler(new Vector3 (180,90,180))).transform.parent = this.transform;
            i = i + 1;
        }
        // Debug.Log(buindingData.Count);
        // Debug.Log(buindingData[0].data.Count);
        ...
        for (int i=0; i < kapilist.Count; i++)
        {
            ...
            {
                if(Vector3.Distance(kapiwall[c].transform.position, removelocation) <= 1)
                {
                    Destroy(kapiwall[c]);
                }
            }
